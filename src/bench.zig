...
fn warmupCaches(test_hands: []const u64) void {
    // Touch lookup tables by performing lookups
    const tables = @import("evaluator/tables.zig");
    // Warm up table caches with representative lookups
    for (0..1024) |i| {
        std.mem.doNotOptimizeAway(tables.lookup(@as(u32, @intCast(i))));
        std.mem.doNotOptimizeAway(tables.flushLookup(@as(u16, @intCast(i % 65536))));
    }
...
pub fn testSingleHand() !void {
    ...
    const slow_result = evaluator.slow.evaluateHand(test_hand);
